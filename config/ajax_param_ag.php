...
if ($_SESSION['Autoriser']!= "oui") {
    ...
    if (isset($_POST['Profil'])) {
        ...
        echo $ProfilName;
        $sql = $database->prepare("UPDATE employe SET PROFIL = ? WHERE IDEMP = ?");  
        $sql->execute(array($ProfilName,$IDAG));
        if ($sql) {
            echo "PROFIL MODIFIE AVEC SUCCES !".$_FILES['Profil'];
        }
    }
    if(isset($_POST['Profession'])) {
        $sql = $database->prepare("UPDATE employe SET PROFESSION = ? WHERE IDEMP = ?");  
        $sql->execute(array($Profession,$IDAG));
        if ($sql) {
            echo "PROFESSION MODIFIEE AVEC SUCCES !";
        }
    }
    if (isset($_POST['Ville']) || isset($_POST['Qtier']) || isset($_POST['Phone']) || isset($_POST['Email'])) {
        $sql = $database->prepare("UPDATE employe SET VILLE = ?, QUARTIER = ?, TEL = ?, EMAIL = ? WHERE IDEMP = ?");
        $sql->execute(array($Ville,$Qtier,$Phone,$Email,$IDAG));
        if ($sql) {
        ...
        if (password_verify($_POST['Last_pass'],$data['PASS'])) {
            if ($_POST['New_Pass'] == $_POST['Config_pass']) {
                if (strlen($_POST['New_Pass']) > 4) {
                    ...
                    $sql = $database->prepare("UPDATE employe SET PASS = ? WHERE IDEMP = ?");
                    $sql->execute(array($Hash_pass,$IDAG));
                    if ($sql) {
